...
object HFileUtils extends Serializable {
  ...
  def leuToLinks(row: Row): Seq[(String, HFileCell)] = {
    ...
    val chVatPaye = rowToCHLinks(row, luKey, ubrn) ++ rowToVatRefsLinks(row, luKey, ubrn) ++ rowToPayeRefLinks(row, luKey, ubrn)
  ...
  private def rowToPayeRefLinks(row: Row, luKey: String, ubrn: String): Seq[(String, HFileCell)] = row.getStringSeq("payerefs").map(_.flatMap(paye =>
    Seq(
  ...
  def generateRurn(row: Row): String = generateUniqueKey
  ...
  def generateLurn(row: Row): String = generateUniqueKey
