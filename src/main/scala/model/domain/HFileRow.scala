...
case class HFileRow(key: String, cells: Iterable[KVCell[String, String]]) {
  ...
  def getValueOrStr(key: String, byKey: Boolean = true, default: String = ""): String =
    getCellValue(key, byKey).getOrElse(default)

  def getCellValue(key: String, byKey: Boolean): Option[String] =
    if (byKey) cells.collectFirst {
      case KVCell(`key`, value) => Option(value)
    }.flatten //.getOrElse(null)
    else cells.collectFirst {
  ...
  override def equals(obj: scala.Any): Boolean = obj match {
    case HFileRow(otherKey, otherCells)
      if (otherKey == this.key) && (this.cells.toSet == otherCells.toSet) => true
    case _ => false
  }
