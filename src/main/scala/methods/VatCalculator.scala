...
object VatCalculator {
  ...
  def calculateVAT(BIDF: DataFrame, payeDF: DataFrame, VatDF: DataFrame)(implicit activeSession: SparkSession ): DataFrame = {
  ...
  def aggregateDF(df:DataFrame)(implicit spark: SparkSession ): DataFrame = {
    val t = "CALCULATED"
